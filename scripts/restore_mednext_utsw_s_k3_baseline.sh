...
SPLITS_JSON="${SPLITS_JSON:-$OUT_DIR/splits.json}"
...
if [[ "${RESUME:-1}" == "1" && -f "$OUT_DIR/last.pt" ]]; then
  cmd+=(--resume-checkpoint "$OUT_DIR/last.pt")
fi
...
if [[ "${RUN_PROXY_SMOKE:-0}" == "1" ]]; then
  BASELINE_CHECKPOINT="$OUT_DIR/best.pt" FORCE=1 bash scripts/run_mednext_utsw_proxy_transport_smoke.sh
fi
