...
def _build_model(checkpoint: dict[str, Any], dataset: UTSWGliomaDataset, args: argparse.Namespace, config: dict[str, Any]) -> CausalSegFormer3D:
...
def _segmentation_loss(logits: Tensor, target: Tensor) -> Tensor:
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if eval_dataset.metadata_encoder is None and not args.allow_missing_metadata:
        raise FileNotFoundError(
            "Causal evaluation needs metadata proxies for proxy and overlap diagnostics. "
    ...
    for batch_idx, batch in enumerate(tqdm(eval_loader, desc=f"causal-eval:{args.split}", leave=False), start=1):
    ...
    metrics.update(factual_batch_metrics and _average_metric_dicts(factual_batch_metrics) or {})
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate causal SegFormer3D with factual, adjusted, proxy, and overlap metrics.")
