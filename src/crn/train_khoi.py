...
from torch.utils.tensorboard import SummaryWriter  # [KHOI] TensorBoard logging
...
def train(config: dict[str, Any]) -> None:
    ...
    writer = SummaryWriter(str(output_dir))  # [KHOI] ghi event file cho TensorBoard
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(log_line)
        # [KHOI] day moi metric so hoc cua epoch vao TensorBoard
        _scalars = {**{f"train/{k}": v for k, v in train_logs.items()},
                    **{f"val/{k}": v for k, v in val_logs.items()}}
        for _tag, _val in _scalars.items():
            if isinstance(_val, (int, float)):
                writer.add_scalar(_tag, float(_val), epoch)
        save_json(
            {
                "epoch": epoch,
                "warmup_factor": warmup_factor,
    ...
    writer.close()  # [KHOI] flush + dong TensorBoard writer
