...
from causal_mednext.backbone import MedNeXtSegmenter, build_mednext_segmenter
from causal_mednext.mechanisms.brats_hierarchy import (
    logit,
    nested_condition_logits_to_outputs,
    region_logits_to_subregion_prior,
    subregion_prob_to_region_prob,
)
from causal_mednext.mechanisms.et_volume_veto import apply_et_volume_veto
...
class CausalMedNeXt(nn.Module):
    ...
    def _logit(probability: Tensor, eps: float = 1e-4) -> Tensor:
        return logit(probability, eps=eps)

    @classmethod
    def _region_logits_to_subregion_prior(cls, region_logits: Tensor) -> Tensor:
        return region_logits_to_subregion_prior(region_logits)

    @staticmethod
    def _subregion_prob_to_region_prob(subregion_prob: Tensor) -> Tensor:
        return subregion_prob_to_region_prob(subregion_prob)
    ...
    def _apply_et_volume_veto(
        ...
        return apply_et_volume_veto(
            logits,
            region_volume_logits,
            num_classes=self.num_classes,
            region_volume_scale=self.region_volume_scale,
            et_volume_veto_scale=self.et_volume_veto_scale,
            et_volume_veto_multiplier=self.et_volume_veto_multiplier,
            et_volume_veto_min_fraction=self.et_volume_veto_min_fraction,
            et_volume_veto_max_bias=self.et_volume_veto_max_bias,
        )
    ...
    def _nested_condition_logits_to_outputs(
        ...
        return nested_condition_logits_to_outputs(raw_region_logits, condition_delta)
